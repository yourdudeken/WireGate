...
export const ServicesTable: React.FC<ServicesTableProps> = ({ services, onAdd, onDelete }) => (
    <div className="glass-card">
        <div className="px-10 py-8 border-b border-white/5 flex justify-between items-center">
            ...
            <button onClick={onAdd} className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg transition-all hover:-translate-y-0.5">
                    ...
                    {services.map(s => (
                        <tr key={s.domain} className="group hover:bg-white/[0.01] transition-all">
                            ...
                            <td className="py-8 px-10 text-center">
                                <span className="inline-block bg-slate-900 border border-white/5 px-3 py-1.5 rounded-xl font-mono text-sm text-slate-400 font-bold group-hover:border-white/10 transition-all">
                                    {s.port}
                                </span>
                            </td>
                            ...
                            <td className="py-8 px-10 text-right">
                                <div className="flex justify-end gap-3">
                                    ...
                                    <button onClick={() => onDelete(s.domain)} className="p-3 bg-rose-600/5 hover:bg-rose-600 hover:text-white rounded-xl text-rose-500 transition-all shadow-sm">
                                        <Trash2 size={18} />
                                    </button>
                                </div>
                            </td>
